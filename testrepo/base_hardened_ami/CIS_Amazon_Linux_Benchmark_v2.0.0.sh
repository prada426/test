...
####################################################################################################
#  Modified CIS Hardening Script
#    Changes (08/15/2016) :
#      - Comment out uninstall of X Window because it uninstalls JDK as a dependency which we need
#      - Change Default umask from 077 to 027 which is minimum required for CIS Benchmark
#      - Comment out installation of AIDE because it is not a requirement
#      - Comment out filesystem integrity check (through AIDE) because it is not a requirement
#      - Comment out "Ensure interactive boot is not enabled" because it is not an issue in AWS
#      - Comment out /etc/hosts.allow and deny config files because AWS uses security groups
#      - Comment out rsyslog and sysng-log configuration because another logging solution is used
#      - Comment out firewall configuration because Security Groups in AWS control Firewall rules
#      - Comment out Password creation and reuse requirements because SSH Key Authentication used
#      - Add line to disable ipv6 to /etc/modprobe.d/CIS.conf
#      - Remove Password expiration and password change warnings.
#
#
#
####################################################################################################
...
if [ "$PROFILE" = "Level 1" ] || [ "$PROFILE" = "Level 2" ]; then
  ...
  # Disabled because this will also remove JDK and AWS API Tools RPMS
  ...
  modprobe -c | grep "^ipv6" || echo "options ipv6 disable=1" >> /etc/modprobe.d/CIS.conf
  ...
  egrep -q "^(\s*)umask\s+\S+(\s*#.*)?\s*$" /etc/bashrc && sed -ri "s/^(\s*)umask\s+\S+(\s*#.*)?\s*$/\1umask 077\2/" /etc/bashrc || echo "umask 027" >> /etc/bashrc
  egrep -q "^(\s*)umask\s+\S+(\s*#.*)?\s*$" /etc/profile && sed -ri "s/^(\s*)umask\s+\S+(\s*#.*)?\s*$/\1umask 077\2/" /etc/profile || echo "umask 027" >> /etc/profile
